...
function CommonWebCore_returnWebElement(strObjXPath) {
	...
	const driver = TCObj.getTcDriver(); // Assumes TCObj.getTcDriver() returns a WebDriver instance
	const waitTime = TSExecParams.getIntMaxWaitTimeSecs() * 1000; // Convert seconds to milliseconds
	...
	if (weElement === null || weElement === undefined) { // Check for null or undefined
		if (boolDoDebug === true) {
...
function CommonWebCore_waitForCountOfBrowserInstances(intCntBrowser, boolCntEquals) {
	...
	const weDriver = TCObj.getTcDriver(); // Assumes TCObj.getTcDriver() returns a WebDriver instance
	...
	if (boolCntEquals === true) {
		...
		let myLoopTime;
		const intMaxWait = TSExecParams.getIntMaxWaitTimeSecs();
...
function CommonWebCore_objHighlightElementJS(weElement, strTestObjType, strTestObjName) {
	...
	const intHighLightCnt = TSExecParams.getIntHighlightCount();
	//Results Variables
	...
	if (weElement !== null && weElement !== undefined) {
		...
		// Original: JavascriptExecutor executor = ((TCObj.getTcDriver()) as JavascriptExecutor)
		//Build loop to execute and wait a standard .25 seconds between flashes
		for (let loopCnt = 0; loopCnt < intHighLightCnt; loopCnt++) {
			driver.executeScript("arguments[0].setAttribute('style','outline: dashed red;');", weElement);
			SeSSleep(200); // Sleep in milliseconds
			driver.executeScript("arguments[0].setAttribute('style','outline: solid green;');", weElement);
			SeSSleep(200); // Sleep in milliseconds
		}
		driver.executeScript("arguments[0].setAttribute('style','outline: none;');", weElement);
...
function CommonWebCore_objVerifyState(weObject, strObjName, boolVisible, boolEnabled) {
	const strGblLineFeed = GVars.GblLineFeed('Value');
	//Create the map
	const mapResults = {};
	let strMethodDetails; // Will be assigned later
	let boolPassed = true;
	...
	const boolIsVisible = weObject.isDisplayed();
	...
	const strObjClass = weObject.getAttribute('class');
	const strObjAriaDisabled = weObject.getAttribute('aria-disabled');
	...
	if (strObjClass === 'unselect-button item item-block item-md item-checkbox item-checkbox-disabled' || strObjAriaDisabled === true) { //Long-term may require a switch
		...
		boolIsEnabled = weObject.isEnabled();
	}
...
function CommonWebCore_objSetEditBoxValue(weEditBox, strObjName, strAssignedValue, boolClearValue, boolVerifyValue, boolTabOutOfField) {
	...
	if (boolClearValue === true) {
		...
		strTestObjText = StrNums.JComm_HandleNoData(weEditBox.getAttribute("value")); // Assumes getAttribute reads value sync
		if (strTestObjText !== gblNull) {
			...
			for (let loop = 0; loop < lenStr; loop++) {
				...
				if (strTestObjText === gblNull) {
					...
					weEditBox.sendKeys(Key.BACK_SPACE); // Use Selenium Key enum
	...
	weEditBox.sendKeys(strTempValue); // WebElement.sendKeys
	DateTime.WaitSecs(TSExecParams.getIntViewDelaySecs()); // Assumes blocking wait
	if (boolTabOutOfField === true) {
		weEditBox.sendKeys(Key.TAB); // Use Selenium Key enum
		DateTime.WaitSecs(TSExecParams.getIntViewDelaySecs()); // Assumes blocking wait
	}
...
function CommonWebCore_objSetMaskEditBoxValue(weEditBox, strObjName, strAssignedValue, boolClearValue, boolInputEncrypted, boolVerifyValue) {
	...
	if (boolInputEncrypted === true) {
		//TODO process the text as encrypted
		//Enter the text using the decrypt call. Do no get value to a variable or check if text matches.
		boolPassed = false;
		strMethodDetails = "FAILED!!! The " + strTestObjType + " " + strObjName + " text of: " + strTestObjText +
			" encryption and decrytion is not currently supported!!!";
	}
	else {
		//Set the value
		let strTempValue;
		...
		if (boolVerifyValue === true) {
			//Retrieve the new value and verify the value matches the assignedValue
			strTestObjText = StrNums.JComm_HandleNoData(weEditBox.getAttribute("value"));
			if (StrNums.JComm_HandleNoData(strTempValue) === strTestObjText) {
...
function CommonWebCore_objVerifyMsgBoxValue(weMsgBox, strObjName, strAssignedValue) {
	...
	let strTestObjText;
	//Retrieve the new value and verify the value matches the assignedValue
	strTestObjText = StrNums.JComm_StripLeadingAssignChars(StrNums.JComm_HandleNoData(weMsgBox.getText().trim())); // Assumes getText() reads text sync
	if (StrNums.JComm_HandleNoData(strAssignedValue) === strTestObjText) {
...
function CommonWebCore_objSetRadioGroupOptionJavaScript(weOptionParent, strObjName, strRadioOptXpath, strRadioGrpXPath, strRadioBtnXPath, strRadioSelXPath, strRadioOptext, boolVerifyValue) {
	...
	if (intCntRDOChildObj > 0) {
		...
		let strOptItemsText = ''; // Initialize to empty string
		let boolRadioOptionFound = false;
		const driver = TCObj.getTcDriver(); // Assumes WebDriver instance
...
function CommonWebCore_waitForPageLoad() {
	const mapMethodResults = {};
	let strMethodDetails; // Will be assigned later
	let boolMethodPassed = false;
	const weDriver = TCObj.getTcDriver(); // Assumes WebDriver instance
	let weLoading; // Placeholder for WebElement

	// Original: Took a try/catch to check if element exists (WebDriver.findElement throws if not found)
	try {
		weLoading = weDriver.findElement(By.xpath(TCObj.getStrLoadingObjXPath()));
	}
	catch (e) {
		weLoading = null; // Set to null if not found
	}

	if (weLoading !== null && weLoading !== undefined) {
		// Original: JavascriptExecutor jsExec = (JavascriptExecutor)weDriver - direct casting
		// In JS, executeScript is directly on the driver
		// Original: String strReadyState = jsExec.executeScript(("return document.readyState").toString())
		let strReadyState = weDriver.executeScript("return document.readyState"); // Assumes executeScript is sync/returns synchronously
		let intLoopCnt;
		if (TSExecParams.getIntMaxWaitTimeSecs() === 0) {
			intLoopCnt = 120; //Hardcoded to 30 seconds with 4 waits per second (120 * 250ms = 30000ms)
		}
		else {
			intLoopCnt = TSExecParams.getIntMaxWaitTimeSecs() * 4; //re-test every 250 ms
		}
		for (let loopWait = 0; loopWait < intLoopCnt; loopWait++) {
			strReadyState = weDriver.executeScript("return document.readyState");
			if (strReadyState === 'complete') {
				// Original: if (weLoading != null) then check displayed. This seems redundant because weLoading is already checked for null.
				// Assuming `weLoading.displayed` is a synchronous check for simplicity.
				if (weLoading.isDisplayed() === false) { // Assuming isDisplayed() returns boolean sync
					boolMethodPassed = true;
					break;
				}
			}
			blockingSleep(250); //wait 250 MS
		}

	}
	else {
		boolMethodPassed = true; // If loading element is not found, assume page is loaded
	}
	if (boolMethodPassed === true) {
...
function CommonWebCore_returnChildElementCount(strParentXpath, strChildXPath) {
	...
	if (weParent !== null && weParent !== undefined) {
		//Return the count of child elements
		const driver = TCObj.getTcDriver(); // Assumes TCObj.getTcDriver() returns a WebDriver instance
		// Original: TCObj.getTcDriver().findElements(By.xpath(strParentXpath + strChildXPath))
...
function waitForPageLoad(driver, loadingXPath, maxWaitTimeSecs = 30) {
    ...
    try {
        weLoading = await driver.findElement(By.xpath(loadingXPath));
    } catch (e) {
        weLoading = null;
    }
    ...
    if (weLoading !== null) {
        ...
        for (let loopWait = 0; loopWait < intLoopCnt; loopWait++) {
            ...
            if (readyState === 'complete') {
                try {
                    const isDisplayed = await weLoading.isDisplayed();
                    if (!isDisplayed) {
                        boolMethodPassed = true;
                        break;
                    }
